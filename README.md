...
<pre>
 image: deviny/fpm:7.1.2-xdebug
 image: deviny/fpm:7.1.2
...
it can help you quickly building basic Laravel development environment by using simple commands: console and create.
...
It would not take port 80 when not be used if not installs DnsMasq.
(OK~Ok~, D-Laravel's idea is inspired by bad experience during using Valet.)

Initial speed is in seconds.

It can simulate different DB environment.

It also could open many different testing hosts.
...
<pre>
./console help (help, console method parameter)
...
`./console alias`  
(If you think ./consoole command is too log, you could create c alias in terminal so that simplifies command to "c info", "c up", or "c down")
...
<pre>
etc/   (nginx、php.ini及mysql settings)
...
If you wish to learn or rebuild self php fpm image version, ex. php extension,
...
| docker-compose ps或docker ps|./console ps  |show docker-compose process   |
